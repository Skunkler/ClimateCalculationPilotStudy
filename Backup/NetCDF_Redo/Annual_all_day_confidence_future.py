...
for root, dirs, files in os.walk(input_dir):
    for filename in files:
        ...
        if filename[-4::1] == '.csv' and 'GCMF' not in root and 'pr' not in filename and 'AnnualAverage' not in root:
            print filename
            ...
            Annual_Average_list = []
            for year_iterator in range(2006, 2101):
                count1 = 1
                sumVar1=0
                for i in lines:
                    if i.split(',')[0] != 'time':
            ...
            for i in range(0, len(Annual_Average_list)):
                ...
                for iterator in lines:
                    ...
                    if int(year) == Year_calc:
                        ...
                        Lower_limit_confidence = (list_data) - (z * Margin_Error)
                        Upper_limit_confidence = (list_data) + (z * Margin_Error)
                        
                line2 = str(Year_calc) + ',' + str(list_data) + ',' + str(day_Count-1) + ',' + str(Variance) + ',' + str(Standard_Deviation) + ',' + \
